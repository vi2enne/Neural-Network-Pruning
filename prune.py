...
from network import CompactCNNPrune
...
parser.add_argument('--filter-percent', type=float, default=0.0,
                    help='scale sparse rate (default: 0.8)')
parser.add_argument('--model', default='checkpoint.s1e-4p1e-4c1e-4.pruning0/model_best.pth.tar', type=str, metavar='PATH',
                    help='path to the model (default: none)')
parser.add_argument('--save', default='./pruning0.0', type=str, metavar='PATH',
...
for k, m in enumerate(model.modules()):
    if isinstance(m, nn.BatchNorm2d):
        if first_BN is not True:
            ...
            cfg_mask.append(torch.ones(1).cuda())
            first_BN = False

# first fc layer
cfg.append(128)
cfg_mask.append(torch.ones(128).cuda())
cfg_mask.append(torch.ones(8).cuda())
...
# Make real prune
print(cfg)
newmodel = CompactCNNPrune(cfg=cfg, filter_percent=args.filter_percent)
if args.cuda:
    newmodel.cuda()
...
for [m0, m1] in zip(model.modules(), newmodel.modules()):
    if isinstance(m0, nn.BatchNorm2d):
        ...
        # for feature map not of size 1x1 
        if first_FC==True:
            idx0_new = []
            idx_list = list(idx0)
